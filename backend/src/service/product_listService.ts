    ...
    async insertIntoList(listId: number, barcode: string, authId: number) {
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                if (error.code === 'P2002') {
                    throw new AppError(
                        'Este produto já foi adicionado a esta lista.',
                        409,
                    );
                }
            }
    ...
    async deleteFromList(
        ...
        try {
            ...
            if (productIsCounted) {
                if (!isAdmin) {
                    throw new AppError(
                        'Este produto possui contagens encerradas para esta lista. Solicite a exclusão para um ADMIN',
                        403,
                    );
                }
                ...
                if (!forceDelete) {
                    throw new AppError(
                        'Este produto possui contagens encerradas para esta lista. Confirme a exclusão para apagar todo o histórico.',
                        409,
                    );
                }
            }
