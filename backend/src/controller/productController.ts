...
export const productController = {
    ...
    async getAllProducts(req: Request, res: Response) {
        try {
            ...
            const isInactiveBool =
                isInactive !== undefined ? isInactive === 'true' : undefined;
        ...
        try {
            ...
            if (!description || !barcode) {
                return res.status(400).json({
                    error: 'Descrição e código de barras são obrigatórios para o cadastro',
                });
            }
        ...
        try {
            ...
            if (!req.isAdmin) {
                return res.status(403).json({
                    error: 'Acesso negado. Apenas usuários ADMIN podem ativar e inativar produtos',
                });
            }
            ...
            if (isInactive === undefined) {
                return res
                    .status(400)
                    .json({ error: 'Verifique a requisição.' });
            }
            ...
            return res
                .status(500)
                .json({ error: 'Erro ao alterar status do produto.' });
        }
    },
};
