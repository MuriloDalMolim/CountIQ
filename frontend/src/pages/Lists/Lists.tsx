...
import { ClipboardList, Plus, ChevronLeft, Filter } from "lucide-react";
...
export function Lists() {
    ...
    const filteredAndSortedLists = lists
        .filter((l) => {
            ...
            const matchesStatus = filterStatus === 'all' ? true : filterStatus === 'active' ? !l.isInactive : l.isInactive
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
            ...
            <PageHeader 
                ...
                subtitle={loading ? "Carregando..." : `Total de ${filteredAndSortedLists.length} listas`}
                icon={<ClipboardList className="w-6 h-6 text-white" />}
                iconBgColor="bg-orange-600"
                action={
                <Button onClick={handleCreate} className="gap-2 bg-brand-500 hover:bg-brand-600 shadow-md">
                ...
                <div className="bg-white p-1 rounded-xl border border-gray-200 shadow-sm flex items-center h-[66px] px-4 gap-2">
                    ...
                    {(['all', 'active', 'inactive'] as const).map((s) => (
                        ...
                        className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${
                            filterStatus === s ? 'bg-blue-50 text-blue-600' : 'text-gray-500 hover:bg-gray-50'
                        }`}
                        >
            ...
            <ListModal 
                ...
                isOpen={isModalOpen}
                onClose={() => { setIsModalOpen(false); setSelectedList(null); }}
            ...
            <ManageProductsModal 
                ...
                onClose={() => { setIsManageModalOpen(false); setSelectedList(null); }}
                list={selectedList}
                onSuccess={loadLists}
            />
            ...
            <ViewProductsModal 
                ...
                onClose={() => { setIsViewModalOpen(false); setSelectedList(null); }}
                list={selectedList}
            />
        </div>
    )
