...
import { ChevronLeft, Loader2, FileText, Printer, AlertCircle } from "lucide-react";
...
export function CountView() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                alert(error.response?.data?.error || "Erro ao carregar relatório.")
            }
            navigate("/counts")
        } finally {
            setLoading(false)
    ...
    const filteredItems = session?.list?.product_list?.filter(p => {
    ...
    if (!session) return (
        <div className="flex flex-col items-center justify-center min-h-screen text-gray-500">
            ...
            <Button onClick={() => navigate("/counts")} className="mt-4">Voltar</Button>
        </div>
    )
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8 pb-24">
            ...
            <PageHeader 
                ...
                action={
                    <Button onClick={() => window.print()} className="bg-gray-800 hover:bg-black gap-2">
            ...
            <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-100">
                        <tr>
                            <th className="px-6 py-4 text-sm font-bold text-gray-600">Produto</th>
                            <th className="px-6 py-4 text-sm font-bold text-gray-600">Cód. Barras</th>
                            <th className="px-6 py-4 text-sm font-bold text-center text-gray-600">Qtd Contada</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                        {filteredItems.map((item) => {
                            const qty = session.count_item.find((ci: CountItemData) => ci.productId === item.productId)?.quantity || 0
                            return (
                                <tr key={item.productId} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <p className="font-bold text-gray-900">
                                            {item.product?.description || item.description || "Sem descrição"}
                                        </p>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className={`inline-block px-3 py-1 rounded-lg font-black text-lg ${qty > 0 ? 'text-brand-600' : 'text-gray-300'}`}>
                                            {qty}
                                        </span>
                                    </td>
                                </tr>
