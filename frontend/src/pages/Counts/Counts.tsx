...
export function Counts() {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            const activeLists = (listsRes.data as List[]).filter(
                (l) => !l.isInactive,
            );
    ...
    const filteredCounts = useMemo(() => {
        return counts.filter((c) => {
            ...
            if (filterOption === 'open')
                return matches && c.status === 'in_progress';
            if (filterOption === 'closed')
                return matches && c.status === 'completed';
            return matches;
        });
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8">
                <div className="flex-1">
                    <SearchBar
                        value={search}
                        onChange={setSearch}
                        placeholder="Buscar por ID ou nome da lista..."
                    />
                </div>
