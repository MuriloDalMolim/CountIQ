...
interface ProductItemProps {
    item: ProductDetail
    initialQty: number
    onUpdate: (productId: number, qty: number, mode: 'increment' | 'set') => Promise<void>
}
...
function ProductItem({ item, initialQty, onUpdate }: ProductItemProps) {
    ...
    const productName = item.product?.description || item.description || "Produto sem descrição"
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all hover:shadow-md">
            <div>
                ...
                <p className="text-sm font-mono text-brand-500">{productBarcode}</p>
            </div>
...
export function Counting() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                alert(error.response?.data?.error || "Erro ao carregar sessão.")
            }
            navigate("/counts")
        } finally {
            setLoading(false)
    ...
    const handleUpdateQuantity = async (productId: number, quantity: number, mode: 'increment' | 'set') => {
        if (!listId || !session) return
        try {
            await api.post(`/countitem/${listId}`, { productId, quantity, mode })
            
            setSession(prev => {
                if (!prev) return null
                const existingIndex = prev.count_item.findIndex(i => i.productId === productId)
                const newCountItems = [...prev.count_item]

                if (existingIndex > -1) {
                    const currentQty = newCountItems[existingIndex].quantity
                    const newQty = mode === 'increment' ? currentQty + quantity : quantity
                    newCountItems[existingIndex] = { ...newCountItems[existingIndex], quantity: Math.max(0, newQty) }
                } else {
                    newCountItems.push({ productId, quantity: Math.max(0, quantity) })
                }
    ...
    const filteredProducts = session?.list?.product_list?.filter(p => {
        const desc = (p.product?.description || p.description || "").toLowerCase()
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8 pb-24">
            ...
            <PageHeader 
                ...
                action={
                    <Button onClick={() => navigate("/counts")} className="bg-green-600 hover:bg-green-700">
                ...
                {filteredProducts.length > 0 ? (
                    filteredProducts.map((item) => (
                        <ProductItem 
                            key={item.productId}
                            item={item}
                            initialQty={session?.count_item.find(i => i.productId === item.productId)?.quantity || 0}
                            onUpdate={handleUpdateQuantity}
                        />
                    ))
                ) : (
                    <div className="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                        <p className="text-gray-400">Nenhum produto listado ou encontrado.</p>
                    </div>
                )}
            </div>
        </div>
